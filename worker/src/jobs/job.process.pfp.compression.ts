...
const R2_OUTPUT_BUCKET = process.env.R2_OUTPUT_BUCKET_PFP!;
const CDN_BASE_URL = process.env.CDN_BASE_URL_PFP ?? "https://pfps.try-yugen.com";

if (!TINIFY_API_KEY) throw new Error("Missing TINIFY_API_KEY");
if (!R2_OUTPUT_BUCKET) throw new Error("Missing R2_OUTPUT_BUCKET_PFP");
...
type Job = { id: string; payload: { auth_id: string } };

/**
 * Flow (no original image bytes on this server):
 *
 *   1. Tinify fetches the pfp from your public CDN URL directly.
 ...
 */
export async function processPfpCompressionJob(job: Job): Promise<void> {
  const authId = job.payload?.auth_id;
  if (!authId) throw new Error("Invalid job payload: missing auth_id");
  ...
  if (userError || !user?.pfp_path) {
    throw new Error(`User not found or missing pfp_path: ${authId}`);
  }

  // ── 2. Submit source URL to Tinify — Tinify fetches the image itself ────────
  const sourceUrl = `${CDN_BASE_URL}/${encodeURIComponent(user.pfp_path)}`;
  logger.info("📸 Submitting pfp to Tinify", { authId, sourceUrl });
  ...
  // ── 3. Build output key ─────────────────────────────────────────────────────
  const originalExt = user.pfp_path.split(".").pop() ?? "jpg";
  const outputKey = `${user.pfp_path}`;
  ...
  //    The original pfp (potentially large) never touches this server — Tinify
  //    fetched it directly from your CDN. Only the already-compressed output
  ...
  logger.info("✅ Compressed pfp uploaded to R2", {
    authId,
    outputKey,
  ...
  logger.info("✅ Pfp compression job complete", {
    jobId: job.id,
    authId,
    outputKey,
