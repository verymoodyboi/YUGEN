...
const Crosshair = ({ color = "white", containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    const tl = gsap
      .timeline({
        paused: true,
        repeat: -1, // loop infinitely while hovered
        yoyo: true, // smooth ping-pong animation
        onStart: () => {
    ...
    // Select all elements with data attribute for glitch effect
    const glitchElements = containerRef?.current
      ? containerRef.current.querySelectorAll("[data-crosshair-glitch]")
      : document.querySelectorAll("[data-crosshair-glitch]");
