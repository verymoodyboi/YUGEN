...
function SignUpForm() {
  ...
  const steps = ["Name", "Aditional Info", "Credentials", "Profile"];
  const validateStep = async () => {
    ...
    if (activeStep === 2) {
      if (!email || !/^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/.test(email))
        return toast.warn("Valid email required"), false;
      const emailErr = await freeEmail();
    ...
    return (
      ...
      >
        ...
        <Typography variant="h5">
          You are all set up! check the verification email we sent you to log
          into your account and start creating and watching movies!{" "}
        </Typography>
      </Box>
    );
  }
