...
const hiddenThoughtIds = new Set(flaggedThoughts?.map((f) => f.thought_id) || []);

// Get IDs of replies flagged by this user
...
const hiddenReplyIds = new Set(flaggedReplies?.map((f) => f.reply_id) || []);
