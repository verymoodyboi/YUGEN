...
export async function processFilmJob(jobRow: any) {
  ...
  try {
    ...
    const filmBuffer = Buffer.from(
      await r2Resp.Body.transformToByteArray()
    );
 ...
 if (posterKey) {
      try {
        ...
        await supabase
          ...
          .update({
            ...
            flag_reason:
              "Poster moderation failed — requires manual review",
          })
          .eq("film_uuid", filmUuid);
      }
    }
    ...
    try {
      ...
      logger.error("Film moderation failed:", err);
    }
